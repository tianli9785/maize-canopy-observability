# -*- coding: utf-8 -*-
"""Minimal reproduction code for M2 WS-CB Transformer-LSTM."""
...
DEVICE="cuda" if torch.cuda.is_available() else "cpu"; COMPONENTS=["WSPC","TB"]
CONFIGS={"spectral": {"feature_mode": "spectral_only", "resolution": "3m", "seed": 715}, "texture": {"feature_mode": "texture_only", "resolution": "1.5m", "seed": 209}, "combined": {"feature_mode": "spectral_texture", "resolution": "1.5m", "seed": 51}}
...
def add_target_formula(df: pd.DataFrame) -> pd.DataFrame:
    """Create the whole-shoot concentration-biomass control targets.

    The control model predicts harvest Target as:
        Target = WSPC * TB
    where:
        TB   = LB + SB + GB
        WSPC = (LPC*LB + SPC*SB + GPC*GB) / TB

    Here WSPC is the biomass-weighted whole-shoot mean P concentration, not the
    arithmetic mean of LPC, SPC and GPC. This keeps the target definition
    consistent with phosphorus mass balance.
    """
    for c in ["LPC", "LB", "SPC", "SB", "GPC", "GB"]:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    # Organ-sum Target is retained as the observed reference target.
    df["Target_formula"] = df["LPC"] * df["LB"] + df["SPC"] * df["SB"] + df["GPC"] * df["GB"]
    df["TB"] = df["LB"] + df["SB"] + df["GB"]
    df["WSPC"] = df["Target_formula"] / (df["TB"] + EPS)
    df.loc[df["TB"] <= 0, "WSPC"] = np.nan
...
class WholeShootTransformerLSTM(nn.Module):
    def __init__(self, seq_dim: int, static_dim: int, hidden_dim=64, n_heads=4, n_layers=1, lstm_layers=1, dropout=0.2):
        ...
        self.head = nn.Sequential(
            ...
            nn.Linear(hidden_dim, len(COMPONENTS))
        )
...
def targets_to_target(Y: np.ndarray) -> np.ndarray:
...
def train_model(Xseq_train, Xst_train, Y_train, Xseq_val, Xst_val, Y_val, model_seed: int):
    ...
    target_train = targets_to_target(Y_train)
    ...
    model = WholeShootTransformerLSTM(
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for xb_seq, xb_st, yb_z, yb_orig in loader:
            ...
            if TARGET_TRANSFORM == "log1p":
                ...
                pred_orig = pred_t
            pred_target = pred_orig[:, 0] * pred_orig[:, 1]
            true_target = yb_orig[:, 0] * yb_orig[:, 1]
        ...
        with torch.no_grad():
            ...
            if TARGET_TRANSFORM == "log1p":
                ...
                val_orig = val_t
            val_target_pred = val_orig[:, 0] * val_orig[:, 1]
            val_target_true = Yval_orig_t[:, 0] * Yval_orig_t[:, 1]
...
def predict_targets(model, y_scaler, Xseq, Xst):
...
def main(feature):
    ...
    ptr_targets=predict_targets(model,yscaler,Xtr,Sctr); pva_targets=predict_targets(model,yscaler,Xva,Scva); ptr=targets_to_target(ptr_targets); pva=targets_to_target(pva_targets)
    ...
    out_dir=os.path.join("outputs",f"M2_WS-CB_{feature}")
    selected=[{"Branch":"sequence","Feature":x} for x in seq]+[{"Branch":"static","Feature":x} for x in sta]
    config={"strategy":"WS-CB","formula":"Target = WSPC * TB","feature":feature,**cfg,"target_transform":TARGET_TRANSFORM,"seq_topk":SEQ_TOP_K,"static_topk":STATIC_TOP_K,"max_abs_collinearity":MAX_CORR_FOR_PRUNE,
